...
def get_detection_inv_matrix(calib_path, frame):
    ...
    # file = os.path.join(calib_path, frame + ".txt")
    return get_inv_matrix(calib_path, "Tr_velo_to_cam", "R0_rect")
...
def parse_one_detection_obj(inv_matrix, l):
    ...
    obj["psr"] = {"scale":
                  ...
                  # "position": {"x": trans_pos[0], "y": trans_pos[1], "z": trans_pos[2]},
                  "rotation": {"x": 0,
                               "y": 0,
...
def trans_detection_label(src_label_path, src_calib_path, tgt_label_path):
    ...
    for fname in files[:10]:
        ...
        bin_file = os.path.join(src_label_path, fname).replace('label_2', 'velodyne').replace('.txt', '.bin')
        pcd_file = os.path.join(tgt_label_path, frame + ".pcd").replace('label', 'lidar')
        bin2pcd(pcd_file, bin_file)
