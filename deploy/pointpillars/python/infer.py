...
def parse_args():
    ...
    parser.add_argument(
        "--det_class",
        type=str,
        help="Detection class, should be 'Car' or 'Person'.",
        required=True)
    parser.add_argument(
        "--model_file",
        type=str,
...
def parse_result(box3d_lidar, label_preds, scores, name, class_type):
    ...
    for box_idx in range(num_bbox3d):
        ...
        if scores[box_idx] < 0:
            continue
        if bbox3d_dims == 7:
            with open(f'result/load1/{name}.txt', "a") as f:
                det_result = "Score: {} Label: {} Box(x_c, y_c, z_c, w, l, h, -rot): {} {} {} {} {} {} {}\n".format(scores[box_idx], label_preds[box_idx],
...
def main(args):
    ...
    # import pdb
    # pdb.set_trace()
    lidar_file = args.lidar_file.split('/')[-1].split('.')[0]
    parse_result(box3d_lidar, label_preds, scores, lidar_file, args.det_class)
...
#     --det_class Car \
#     --model_file model/Car/output/pointpillars.pdmodel \
#     --params_file model/Car/output/pointpillars.pdiparams \
